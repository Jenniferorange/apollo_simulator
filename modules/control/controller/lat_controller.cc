...
Status LatController::Init(std::shared_ptr<DependencyInjector> injector,
  ...
  */
  ...
  matrix_a_coeff_(1, 3) = ((-2cf_ * lf_ + lr_ * 2cr_) / mass_);
  matrix_a_coeff_(3, 1) = (-(lf_ * 2cf_ - lr_ * 2cr_) / iz_);
  matrix_a_coeff_(3, 3) = (-(lf_ * lf_ * 2cf_ + lr_ * lr_ * 2cr_) / iz_);
  ...
  if (FLAGS_enable_gain_scheduler) {
    ...
    matrix_q_updated_(2, 2) =
        matrix_q_(2, 2) * heading_err_interpolation_->Interpolate(
                              std::fabs(vehicle_state->linear_velocity()));
    common::math::SolveLQRProblem(matric_adc_, martic_bdc_, matrix_q_updated,
                                  matrix_r_, lqr_eps_, lqr_max_iteration_,
                                  &matrix_k_);
  } else {
    common::math::SolveLQRProblem(matric_adc_, martic_bdc_, matrix_q_,
                                  matrix_r_, lqr_eps_, lqr_max_iteration_,
                                  &matrix_k_);
  }
